...
public static class ListSiriVersionsCommand
{
    public static Command Create()
    {
        var command = new Command("list-siri-versions", "List available SIRI schema versions from the official repository");

        command.SetAction(async (_, cancellationToken) =>
        {
            try
            {
                Console.WriteLine("Fetching available SIRI schema versions...");
                Console.WriteLine();

                var versions = await SchemaDownloader.ListVersionsAsync(
                    SiriDefaults.GitHubApiTagsUrl,
                    cancellationToken);
